    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="grid grid-cols-3 gap-2 text-center sm:min-w-[420px]">
          <div className="rounded-2xl border border-luxury-border bg-luxury-dark/60 px-4 py-3">
            ...
            <p className="mt-1 text-xl font-semibold text-luxury-white">{totalOrders}</p>
          </div>
          <div className="rounded-2xl border border-luxury-border bg-luxury-dark/60 px-4 py-3">
            <p className="text-xs uppercase tracking-[0.16em] text-luxury-muted">Page value</p>
            <p className="mt-1 text-xl font-semibold text-luxury-white">{formatPrice(totalRevenue)}</p>
          </div>
          <div className="rounded-2xl border border-luxury-border bg-luxury-dark/60 px-4 py-3">
            <p className="text-xs uppercase tracking-[0.16em] text-luxury-muted">Need deposit</p>
            <p className="mt-1 text-xl font-semibold text-luxury-white">{statusCounts.find((s) => s.status === OrderStatus.DEPOSIT_PENDING)?._count ?? 0}</p>
          </div>
        </div>
      </header>
      ...
      <Card className="rounded-2xl">
        <form action="/admin/orders" className="grid gap-3 lg:grid-cols-[minmax(240px,1fr)_180px_180px_150px_150px_auto_auto]">
          ...
          <Link href="/admin/orders" className="inline-flex items-center justify-center rounded-lg border border-luxury-border px-5 py-2.5 text-sm font-medium text-luxury-muted transition hover:text-luxury-white">
            Reset
          </Link>
        </form>
      </Card>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.1fr_0.8fr_0.7fr_auto] xl:items-center">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <Link href={`/admin/orders/${order.id}`} className="font-medium text-luxury-white hover:text-gold-300">
                      {order.orderNumber}
                    </Link>
                ...
                <div className="rounded-xl border border-luxury-border bg-luxury-dark/50 p-3 text-sm">
                  <div className="flex justify-between gap-6"><span className="text-luxury-muted">Total</span><span className="text-luxury-white">{formatPrice(order.total)}</span></div>
      ...
      <div className="flex items-center justify-between text-sm text-luxury-muted">
        ...
        <div className="flex gap-2">
          {page > 1 ? (
            <Link href={ordersHref({ q, status, payment: paymentStatus, from, to, page: page - 1 })} className="rounded-xl border border-luxury-border px-4 py-2 transition hover:text-luxury-white">Previous</Link>
          ) : <span className="rounded-xl border border-luxury-border px-4 py-2 opacity-40">Previous</span>}
          {page < pageCount ? (
            <Link href={ordersHref({ q, status, payment: paymentStatus, from, to, page: page + 1 })} className="rounded-xl border border-luxury-border px-4 py-2 transition hover:text-luxury-white">Next</Link>
