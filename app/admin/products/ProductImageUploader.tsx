...
export default function ProductImageUploader({
  ...
  useEffect(() => {
    ...
    const handleSubmit = (event: SubmitEvent) => {
      ...
      if (images.filter((image) => image.url && !image.error).length === 0) {
        event.preventDefault();
          ...
          {images.map((image, index) => (
            <article key={image.localId} className="overflow-hidden rounded-2xl border border-luxury-border bg-luxury-dark/40">
              <div className="relative aspect-square bg-luxury-dark">
                {/* eslint-disable-next-line @next/next/no-img-element -- Admin previews include local object URLs during upload. */}
              ...
              <div className="space-y-3 p-4">
                {image.error && <p className="text-xs text-gold-300">{image.error}</p>}
                <input
                  value={image.altText ?? ""}
