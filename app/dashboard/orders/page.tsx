            ...
            {orders.map((order) => (
              <Card key={order.id} className="rounded-2xl">
                ...
                <p className="mt-2 text-xs text-luxury-muted">
                  Livraison : {order.trackingNumber ?? order.address?.city ?? "En préparation"}
                </p>
              </Card>
            ))}
          </div>
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-luxury-dark text-xs uppercase tracking-[0.16em] text-luxury-muted">
                  <tr>
                    ...
                    <th className="px-5 py-4 font-medium">Livraison</th>
                ...
                <tbody>
                  {orders.map((order) => (
                    <tr key={order.id} className="border-t border-luxury-border transition-colors hover:bg-luxury-border/20">
                      ...
                      <td className="px-5 py-4 text-luxury-muted">{order.trackingNumber ?? order.address?.city ?? "En préparation"}</td>
                      <td className="px-5 py-4 text-right font-medium text-white">{formatPrice(order.total)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
